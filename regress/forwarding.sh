...
${SSH} -F $OBJ/ssh_config -p$last -oConnectionAttempts=10 \
...
${SSH} -F $OBJ/ssh_config -p${base}02 -oConnectionAttempts=10 \
	somehost cat ${DATA} > ${COPY}
test -s ${COPY}		|| fail "failed copy of ${DATA}"
cmp ${DATA} ${COPY}	|| fail "corrupted copy of ${DATA}"
...
${SSH} -F $OBJ/ssh_config -p${base}01 -oConnectionAttempts=10 \
	somehost cat ${DATA} > ${COPY}
