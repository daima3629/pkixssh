...
/*
 * Arg p must be dynamically allocated.  sftp_make_absolute will either return it
    ...
    const char *startdir, int err_abort, int echo_command)
{
	...
	case I_LINK:
		if (!sflag)
			path1 = sftp_make_absolute(path1, *pwd);
		path2 = sftp_make_absolute(path2, *pwd);
		err = (sflag ? sftp_symlink : sftp_hardlink)(conn, path1, path2);
		break;
	case I_RM:
...
interactive_loop(struct sftp_conn *conn, char *file1, char *file2)
{
	...
	remote_path = sftp_realpath(conn, ".");
	if (remote_path == NULL)
		fatal("Need cwd");
	...
	err = 0;
	for (;;) {
	{	/* manage command line interrupts */
		struct sigaction sa;
