...
mux_master_session_cleanup_cb(struct ssh *ssh, int cid, int force, void *arg)
{
	...
	if (c == NULL)
		fatal_f("channel_by_id(%d) == NULL", cid);
	if (c->ctl_chan != -1) {
    ...
    Channel *c, struct sshbuf *m, struct sshbuf *reply)
{
	...
	if (ver != SSHMUX_VER) {
		error_f("unsupported multiplexing protocol version %u "
		    "(expected %u)", (int)ver, SSHMUX_VER);
		return -1;
	}
	debug2_f("channel %d client version %u", c->self, (unsigned)ver);
    ...
    Channel *c, struct sshbuf *m, struct sshbuf *reply)
{
	...
	if ((r = sshbuf_get_u32(m, &ftype)) != 0 ||
	    ...
	    (lport != (u_int32_t)PORT_STREAMLOCAL && lport > 65535) ||
	    (cport != (u_int32_t)PORT_STREAMLOCAL && cport > 65535)) {
		error_f("malformed message");
		ret = -1;
		goto out;
	}
	...
	if (ftype != MUX_FWD_LOCAL && ftype != MUX_FWD_REMOTE &&
		...
		error_f("invalid forwarding type %u", (unsigned)ftype);
 invalid:
		free(listen_addr);
		free(connect_addr);
		reply_error(reply, MUX_S_FAILURE, rid,
	...
	if (ftype == MUX_FWD_DYNAMIC && fwd.listen_path) {
		error_f("streamlocal and dynamic forwards "
		    "are mutually exclusive");
		goto invalid;
	}
	if (fwd.listen_port != PORT_STREAMLOCAL && fwd.listen_port >= 65536) {
		error_f("invalid listen port %u", fwd.listen_port);
		goto invalid;
	}
	if ((fwd.connect_port != PORT_STREAMLOCAL &&
	    ...
	    fwd.connect_port == 0)) {
		error_f("invalid connect port %u", fwd.connect_port);
		goto invalid;
	}
	if (ftype != MUX_FWD_DYNAMIC && fwd.connect_host == NULL &&
	    fwd.connect_path == NULL) {
		error_f("missing connect host");
		goto invalid;
	}
 ...
 exists:
	...
	if (ftype == MUX_FWD_LOCAL || ftype == MUX_FWD_DYNAMIC) {
		if (!channel_setup_local_fwd_listener(ssh, &fwd,
		    &options.fwd_opts)) {
 fail:
			error_f("requested %s failed", fwd_desc);
			reply_error(reply, MUX_S_FAILURE, rid,
    ...
    Channel *c, struct sshbuf *m, struct sshbuf *reply)
{
	...
	debug2_f("channel %d: stdio fwd to %s:%u", c->self, chost, (unsigned)cport);
...
mux_master_read_cb(struct ssh *ssh, Channel *c)
{
 ...
 malf:
	...
	if (type == MUX_MSG_HELLO)
		...
		if (!state->hello_rcvd) {
			error_f("expected MUX_MSG_HELLO(0x%08x), "
			    "received 0x%08x", MUX_MSG_HELLO, (unsigned)type);
			goto out;
		}
	...
	if (mux_master_handlers[i].handler == NULL) {
		error_f("unsupported mux message 0x%08x", (unsigned)type);
...
muxserver_listen(struct ssh *ssh)
{
 ...
 disable_mux_master:
	...
	mux_listener_channel = channel_new(ssh, "mux listener",
	    ...
	    0, options.control_path, CHANNEL_NONBLOCK_SET);
...
mux_client_hello_exchange(int fd)
{
	...
	if (type != MUX_MSG_HELLO) {
		error_f("expected HELLO (%u) got %u", MUX_MSG_HELLO, (unsigned)type);
		goto out;
	}
	if ((r = sshbuf_get_u32(m, &ver)) != 0)
	...
	if (ver != SSHMUX_VER) {
		error("Unsupported multiplexing protocol version %d "
		    "(expected %d)", (int)ver, SSHMUX_VER);
		goto out;
	}
	debug2_f("master version %u", (unsigned)ver);
	/* No extensions are presently defined */
	while (sshbuf_len(m) > 0) {
		char *name = NULL;
		...
		debug2_f("unrecognised master extension \"%s\"", name);
		free(name);
	}
	/* success */
	ret = 0;
...
mux_client_request_alive(int fd)
{
	...
	if (rid != muxclient_request_id)
		fatal_f("out of sequence reply: my id %u theirs %u",
		    muxclient_request_id, (unsigned)rid);
	if ((r = sshbuf_get_u32(m, &pid)) != 0)
		fatal_fr(r, "parse PID");
	sshbuf_free(m);

	debug3_f("done pid = %u", (unsigned)pid);
...
mux_client_request_terminate(int fd)
{
	...
	default:
		fatal_f("unexpected response from master 0x%08x", (unsigned)type);
	}
	sshbuf_free(m);
	muxclient_request_id++;
}
...
mux_client_forward(int fd, int cancel_flag, u_int ftype, struct Forward *fwd)
{
	...
	{	u_int32_t val;
		if ((r = sshbuf_get_u32(m, &val)) != 0)
			fatal_fr(r, "parse port");
		fwd->allocated_port = val;
	}
...
mux_client_request_session(int fd)
{
	...
	case MUX_S_SESSION_OPENED:
		...
		debug_f("master session id: %u", (unsigned)sid);
		break;
	case MUX_S_PERMISSION_DENIED:
		if ((r = sshbuf_get_cstring(m, &e, NULL)) != 0)
			fatal_fr(r, "parse error message");
	...
	default:
		sshbuf_free(m);
		error_f("unexpected response from master 0x%08x", (unsigned)type);
		return -1;
	}
	muxclient_request_id++;
	...
	/*
	 * Stick around until the controler closes the client_fd.
		...
		case MUX_S_TTY_ALLOC_FAIL:
			...
			if (esid != sid)
				fatal_f("tty alloc fail on unknown session: "
				    "my id %u theirs %u", (unsigned)sid, (unsigned)esid);
	...
	if (muxclient_terminate) {
		...
		debug2("Received exit status from master %d", (int)exitval);
