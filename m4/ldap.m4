...
if test "x$ac_ldap" = xyes; then
  ...
  AC_ARG_WITH([ldap-prefix],
    [AS_HELP_STRING(
      [--with-ldap-prefix=PATH],
      [Prefix where LDAP is installed (optional)]
    )],
  ...
  AC_ARG_WITH([ldap-bindir],
    [AS_HELP_STRING(
      [--with-ldap-bindir=PATH],
      [Prefix where LDAP user executables are installed (optional)]
    )],
    [LDAP_BINDIR=$withval],
    [
  ...
  AC_ARG_WITH([ldap-libexecdir],
    [AS_HELP_STRING(
      ...
      [Prefix where LDAP program executables are installed (optional)]
    )],
...
dnl  fi
  ...
  AC_ARG_WITH([ldap-sysconfdir],
    [AS_HELP_STRING(
      ...
      [Prefix where LDAP single-machine data are installed (optional)]
    )],
...
dnl  fi
  ...
  AC_ARG_WITH([ldap-includedir],
    [AS_HELP_STRING(
      ...
      [Prefix where LDAP header files are installed (optional)]
    )],
  ...
  AC_ARG_WITH([ldap-libs],
    [AS_HELP_STRING(
      [--with-ldap-libs=LIBS],
      [Specify LDAP libraries to link with (default is -lldap -llber -lssl -lcrypto)]
    )],
