...
send_msg(struct sftp_conn *conn, struct sshbuf *m)
{
	...
	iov[1].iov_base = sshbuf_mutable_ptr(m);
...
get_decode_stat(struct sftp_conn *conn, u_int expected_id, int quiet, Attrib *attr)
{
	...
	if (type == SSH2_FXP_STATUS) {
		...
		if ((r = sshbuf_get_u32(msg, &status)) != 0)
			fatal_fr(r, "parse status");
		if (quiet)
			debug("remote stat: %s", fx2txt(status));
		else
...
sftp_close(struct sftp_conn *conn, const u_char *handle, u_int handle_len)
{
	...
	if (status != SSH2_FX_OK)
		error("remote close: %s", fx2txt(status));
...
sftp_symlink(struct sftp_conn *conn, const char *oldpath, const char *newpath)
{
	...
	status = get_status(conn, id);
	if (status != SSH2_FX_OK)
		error("remote symlink \"%s\" to \"%s\": %s", oldpath,
		    newpath, fx2txt(status));
	...
	while (num_req > 0 || max_req > 0) {
		...
		case SSH2_FXP_DATA:
			...
			if ((lseek(local_fd, req->offset, SEEK_SET) == -1 ||
				 ...
				 * acknowledged.
				 * Letter is needed to truncate interrupted
				 * transfers.
				 */
				if (maxack < req->offset + len)
					maxack = req->offset + len;
				if (!reordered) {
					if (req->offset <= highwater)
						highwater = maxack;
					else {
    ...
    int fsync_flag, int inplace_flag)
{
	...
	if (fstat(local_fd, &sb) == -1) {
		error("local fstat \"%s\": %s", local_path, strerror(errno));
		close(local_fd);
		return(-1);
	}
	if (!S_ISREG(sb.st_mode)) {
		error("local \"%s\": not a regular file", local_path);
		close(local_fd);
		return(-1);
	}
	stat_to_attrib(&sb, &a);
	...
	for (;;) {
		...
		if (id == startid || len == 0 ||
			 ...
			 * Letter is needed to truncate interrupted transfers.
			 */
			if (maxack < ack->offset + ack->len)
				maxack = ack->offset + ack->len;
			if (!reordered) {
				if (ack->offset <= highwater)
					highwater = maxack;
				else {
					debug3_f("server reordered ACKs");
					reordered = 1;
	...
	if (close(local_fd) == -1) {
		error("local close \"%s\": %s", local_path, strerror(errno));
		status = SSH2_FX_FAILURE;
	}
    ...
    int follow_link_flag, int inplace_flag)
{
	...
	if (stat(src, &sb) == -1) {
		error("local stat \"%s\": %s", src, strerror(errno));
		return -1;
	}
	if (!S_ISDIR(sb.st_mode)) {
		error("local \"%s\": not a directory", src);
		return -1;
	}
	if (print_flag && print_flag != SFTP_PROGRESS_ONLY)
		mprintf("Entering %s\n", src);
...
{	u_int32_t saved_perm = a.perm;
	...
	while (((dp = readdir(dirp)) != NULL) && !interrupted) {
		...
		if (lstat(new_src, &sb) == -1) {
			...
			logit("local \"%s\": not a regular file", filename);
	}
	free(new_dst);
	free(new_src);
